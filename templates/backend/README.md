...
For example, a Python service can use `python:3.12.4-slim` images, `pip install -r requirements.txt` as `INSTALL_COMMAND`, and `python -m app` as `START_COMMAND`; Go, Java, PHP, Ruby, and other languages use the same contract with their own official pinned images and commands. The selected runtime image must provide `/bin/sh`; the runtime preparation command must copy only the files needed at runtime into `/runtime`.

---

## Architecture

Runtime shape: one language-agnostic `api` service and one `db` service (PostgreSQL), on a private `app-name-internal` network, with `api` additionally attached to `edge` and routed by Traefik. See [ARCH-002 — Platform Architecture](../../docs/01-architecture/ARCH-002-platform-architecture.md) for how this fits into the broader platform, and [ARCH-004 — Network Architecture](../../docs/01-architecture/ARCH-004-network-architecture.md) for the network model this `compose.yaml` implements.
...
Run the application locally using your framework's standard tooling against a local PostgreSQL instance. Local development tooling is owned by this repository, not the platform.
