...
Python, Go, Java, PHP, Ruby, and other bot implementations use the same contract with their own official pinned images and commands. The selected runtime image must provide `/bin/sh`; the runtime preparation command must copy only the files needed at runtime into `/runtime`.
