...
current_version() {
  ...
  "${RCLONE_TARGET}" version 2>/dev/null | awk '/^rclone v/ { sub(/^v/, "", $2); print $2; exit }'
}

download_release_file() {
  local name="$1" destination="$2"
  echo "Primary download: ${github_base}/${name}"
  if curl "${curl_args[@]}" "${github_base}/${name}" -o "${destination}"; then
    return 0
  fi
  echo "Primary download failed or timed out; trying official fallback." >&2
  curl "${curl_args[@]}" "${official_base}/${name}" -o "${destination}"
}

verify_archive() {
  local checksum_file="$1" archive_dir="$2" checksum_line
  checksum_line="$(awk -v hash="${RCLONE_SHA256}" -v name="${RCLONE_ASSET}" \
    '$1 == hash && ($2 == name || $2 == "*" name) { print; exit }' "${checksum_file}")"
  [ -n "${checksum_line}" ] || {
...
install_rclone() {
  local temporary_dir archive_dir extract_dir archive checksum_file installed
  ...
  download_release_file "${RCLONE_ASSET}" "${archive}"
  download_release_file SHA256SUMS "${checksum_file}"
  verify_archive "${checksum_file}" "${archive_dir}"
  echo "Checksum verified for ${RCLONE_ASSET}."

  unzip -q "${archive}" -d "${extract_dir}"
  [ -x "${extract_dir}/rclone" ] || {
    echo "Release archive does not contain an executable rclone binary." >&2
    return 1
  }
  install -d -m 0755 "${RCLONE_INSTALL_DIR}"
  install -m 0755 "${extract_dir}/rclone" "${RCLONE_TARGET}.tmp"
  mv -f -- "${RCLONE_TARGET}.tmp" "${RCLONE_TARGET}"
