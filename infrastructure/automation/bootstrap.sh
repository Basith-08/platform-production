...
require_root() {
  [ "$(id -u)" -eq 0 ] || { echo "Run this command as root." >&2; return 1; }
}

validate_host() {
  require_root
  [ -r /etc/os-release ] || { echo "/etc/os-release is missing." >&2; return 1; }
  ...
  getent hosts github.com >/dev/null || { echo "DNS lookup for github.com failed." >&2; return 1; }
  getent hosts download.docker.com >/dev/null || { echo "DNS lookup for download.docker.com failed." >&2; return 1; }
}

validate_public_key() {
  local label="$1" key_file="$2"
  [ -s "${key_file}" ] || { echo "${label} public key is missing or empty: ${key_file}" >&2; return 1; }
...
install_base_packages() {
  ...
  apt-get update -y
  apt-get install -y ca-certificates curl cron git gnupg rsync ufw unattended-upgrades util-linux unzip openssh-server
...
configure_runtime_layout() {
  ...
  mkdir -p /srv/platform/backup/staging /srv/platform/.staging /srv/apps
  install -d -o "${DEPLOY_USER}" -g "${DEPLOY_USER}" -m 0755 /run/lock/platform-backup
  install -d -o "${DEPLOY_USER}" -g "${DEPLOY_USER}" -m 0750 /var/log/platform
  touch /var/log/platform/backup.log /var/log/platform/maintenance.log
...
provision() {
  ...
  while [ "$#" -gt 0 ]; do
    option="$1"
    case "${option}" in
      --admin-key) [ "$#" -ge 2 ] || { echo "--admin-key needs a path." >&2; return 2; }; admin_key="$2"; shift 2 ;;
      --deploy-key) [ "$#" -ge 2 ] || { echo "--deploy-key needs a path." >&2; return 2; }; deploy_key="$2"; shift 2 ;;
      --hostname) [ "$#" -ge 2 ] || { echo "--hostname needs a value." >&2; return 2; }; hostname_name="$2"; shift 2 ;;
      -h|--help) usage; return 0 ;;
      *) echo "Unknown provision option: ${option}" >&2; usage >&2; return 2 ;;
    esac
  done
  ...
  step "Validate Ubuntu 24.04 host" validate_host
  step "Configure admin and deploy identities" configure_identity "${hostname_name}" "${admin_key}" "${deploy_key}"
  step "Install base packages" install_base_packages
  step "Install pinned rclone" "${SCRIPT_DIR}/install-rclone.sh"
  step "Install Docker Engine and Compose" install_docker
  step "Configure firewall and security updates" configure_firewall_and_updates
  step "Create runtime layout and logging limits" configure_runtime_layout
  step "Run provisioning postflight" postflight
...
EOF
  ...
  if ! /usr/sbin/sshd -t; then
    if [ "${had_previous}" -eq 1 ]; then cp -p "${backup_file}" "${SSH_HARDENING_FILE}"; else rm -f "${SSH_HARDENING_FILE}"; fi
...
case "${command_name}" in
  provision) provision "$@" ;;
  harden) [ "$#" -eq 0 ] || { echo "harden accepts no options." >&2; usage >&2; exit 2; }; harden ;;
  -h|--help) usage ;;
  *)
